...
parser.add_argument("-m", "--max_anchor_coverage", help="max kcov value for anchormap (default: 0, uses map without anchor filtering)", type=int, default=0)
    ...
    for nid in to_split:
        ...
        for ci,cstart in enumerate(splits):
            if ci==len(splits)-1: 
                ...
                new_nid=ws.sdg.add_node(seq[cstart:splits[ci+1]+62],False)
            if last_nid:
                ws.sdg.add_link(-last_nid,new_nid,-62)
            else:
                for lb in nv.prev():
...
if args.max_anchor_coverage:
    ...
    lrr.anchormap(kcname='main31', countname='pe', fmax=args.max_anchor_coverage, graph_fmin=1, graph_fmax=1)
else: lrr.map()
